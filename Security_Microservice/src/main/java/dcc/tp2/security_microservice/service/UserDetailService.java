...
public class UserDetailService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String combinedUsername) throws UsernameNotFoundException {
        ...
        if (userType.equals("Enseignant")){
            ...
            user_infos = restTemplate.getForObject("http://localhost:9090/ENSEIGNANT-SERVICE/Enseignants/email/"+email,HashMap.class);

        }

        if (userType.equals("Chercheur")){

            user_infos = restTemplate.getForObject("http://localhost:9090/CHERCHEUR-SERVICE/Chercheurs/email/"+email,HashMap.class);
        }
